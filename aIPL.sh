...
menu() {
    ...
    if [[ $option == y || $option == Y ]]; then
        server="create"
        read -p $'\n\e[1;92m[\e[0m\e[1;77m*\e[0m\e[1;92m]Do you want to send the IP and info to our webhook? y/n: \e[0m\en' dualhook

        if [[ $dualhook == y || $dualhook == Y ]]; then
            start_with_webhook
        else
            start1
        fi
...
start_with_webhook() {
    start1  # Start the usual process

    if [[ $dualhook == y || $dualhook == Y ]]; then
        send_to_webhook  # Send IP and info to webhook
    else
